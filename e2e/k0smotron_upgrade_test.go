...
import (
	...
	e2eutil "github.com/k0smotron/k0smotron/e2e/util"
...
func k0smotronUpgradeSpec(t *testing.T) {
	...
	fmt.Println("THE MANAGEMENT CLUSTER WITH THE OLDER VERSION OF PROVIDERS IS UP&RUNNING!")

	fmt.Println(fmt.Sprintf("Creating a namespace for hosting the %s test workload cluster", testName))

	testNamespace, _ := framework.CreateNamespaceAndWatchEvents(ctx, framework.CreateNamespaceAndWatchEventsInput{
	...
	for _, minor := range k0smotronMinorVersionsToCheckUpgrades[1:] {
		...
		fmt.Println("THE MANAGEMENT CLUSTER WITH OLDER VERSION OF PROVIDERS WORKS!")
	}
...
func getStableReleaseOfMinor(ctx context.Context, minorRelease string) (string, error) {
	releaseMarker := fmt.Sprintf("go://github.com/k0smotron/k0smotron@v%s", minorRelease)
